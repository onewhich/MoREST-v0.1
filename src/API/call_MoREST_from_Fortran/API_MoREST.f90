...
interface
    subroutine call_morest_bias_sampling(if_initial, simulation_temperature, potential_energy,&
                       current_md_step, md_force, md_force_shape,&
                       coordinate, coordinate_shape) bind (c)
        use iso_c_binding
        integer(c_int64_t) :: if_initial, current_md_step, md_force_shape(2), coordinate_shape(2)
...
integer(c_int64_t), public :: if_initial, current_md_step, md_force_shape(2), coordinate_shape(2)
